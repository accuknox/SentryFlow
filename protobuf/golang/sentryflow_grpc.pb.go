...
func RegisterSentryFlowServer(s grpc.ServiceRegistrar, srv SentryFlowServer) {
	// If the following call panics, it indicates UnimplementedSentryFlowServer was
...
const (
	APIObserverService_GetAPIEvents_FullMethodName  = "/protobuf.APIObserverService/GetAPIEvents"
	APIObserverService_GetAPIMetrics_FullMethodName = "/protobuf.APIObserverService/GetAPIMetrics"
)

// APIObserverServiceClient is the client API for APIObserverService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// APIObserverService provides KubeArmor-specific API event streaming.
type APIObserverServiceClient interface {
	GetAPIEvents(ctx context.Context, in *APIEventFilter, opts ...grpc.CallOption) (grpc.ServerStreamingClient[APIEvent], error)
	GetAPIMetrics(ctx context.Context, in *MetricsRequest, opts ...grpc.CallOption) (*APIObserverMetrics, error)
}

type aPIObserverServiceClient struct {
	cc grpc.ClientConnInterface
}

func NewAPIObserverServiceClient(cc grpc.ClientConnInterface) APIObserverServiceClient {
	return &aPIObserverServiceClient{cc}
}

func (c *aPIObserverServiceClient) GetAPIEvents(ctx context.Context, in *APIEventFilter, opts ...grpc.CallOption) (grpc.ServerStreamingClient[APIEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &APIObserverService_ServiceDesc.Streams[0], APIObserverService_GetAPIEvents_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[APIEventFilter, APIEvent]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type APIObserverService_GetAPIEventsClient = grpc.ServerStreamingClient[APIEvent]

func (c *aPIObserverServiceClient) GetAPIMetrics(ctx context.Context, in *MetricsRequest, opts ...grpc.CallOption) (*APIObserverMetrics, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(APIObserverMetrics)
	err := c.cc.Invoke(ctx, APIObserverService_GetAPIMetrics_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// APIObserverServiceServer is the server API for APIObserverService service.
// All implementations must embed UnimplementedAPIObserverServiceServer
// for forward compatibility.
//
// APIObserverService provides KubeArmor-specific API event streaming.
type APIObserverServiceServer interface {
	GetAPIEvents(*APIEventFilter, grpc.ServerStreamingServer[APIEvent]) error
	GetAPIMetrics(context.Context, *MetricsRequest) (*APIObserverMetrics, error)
	mustEmbedUnimplementedAPIObserverServiceServer()
}

// UnimplementedAPIObserverServiceServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedAPIObserverServiceServer struct{}

func (UnimplementedAPIObserverServiceServer) GetAPIEvents(*APIEventFilter, grpc.ServerStreamingServer[APIEvent]) error {
	return status.Error(codes.Unimplemented, "method GetAPIEvents not implemented")
}
func (UnimplementedAPIObserverServiceServer) GetAPIMetrics(context.Context, *MetricsRequest) (*APIObserverMetrics, error) {
	return nil, status.Error(codes.Unimplemented, "method GetAPIMetrics not implemented")
}
func (UnimplementedAPIObserverServiceServer) mustEmbedUnimplementedAPIObserverServiceServer() {}
func (UnimplementedAPIObserverServiceServer) testEmbeddedByValue()                            {}

// UnsafeAPIObserverServiceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to APIObserverServiceServer will
// result in compilation errors.
type UnsafeAPIObserverServiceServer interface {
	mustEmbedUnimplementedAPIObserverServiceServer()
}

func RegisterAPIObserverServiceServer(s grpc.ServiceRegistrar, srv APIObserverServiceServer) {
	// If the following call panics, it indicates UnimplementedAPIObserverServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&APIObserverService_ServiceDesc, srv)
}

func _APIObserverService_GetAPIEvents_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(APIEventFilter)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(APIObserverServiceServer).GetAPIEvents(m, &grpc.GenericServerStream[APIEventFilter, APIEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type APIObserverService_GetAPIEventsServer = grpc.ServerStreamingServer[APIEvent]

func _APIObserverService_GetAPIMetrics_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(MetricsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(APIObserverServiceServer).GetAPIMetrics(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: APIObserverService_GetAPIMetrics_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(APIObserverServiceServer).GetAPIMetrics(ctx, req.(*MetricsRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// APIObserverService_ServiceDesc is the grpc.ServiceDesc for APIObserverService service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var APIObserverService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "protobuf.APIObserverService",
	HandlerType: (*APIObserverServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "GetAPIMetrics",
			Handler:    _APIObserverService_GetAPIMetrics_Handler,
		},
	},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "GetAPIEvents",
			Handler:       _APIObserverService_GetAPIEvents_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "sentryflow.proto",
